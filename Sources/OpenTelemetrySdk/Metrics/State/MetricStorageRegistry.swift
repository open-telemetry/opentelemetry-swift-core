    ...
    guard let storage = registry[descriptor] else {
      ...
      for existingStorage in registry.values {
        ...
        if existing.name.lowercased() == descriptor.name.lowercased(), existing != descriptor {
          print("Found duplicate metric definition: \(descriptor.name). A metric with the same name but different identifying fields is already registered. To resolve, consider using a View to rename one of the instruments.")
          break
        }
      }
